...
insert into public.student_guardians(student_id,guardian_id,relationship,is_primary_contact) values ('c0000000-0000-0000-0000-000000000008','d0000000-0000-0000-0000-000000000007','Guardian',true),('c0000000-0000-0000-0000-000000000010','d0000000-0000-0000-0000-000000000009','Guardian',true);
...
insert into public.academic_years(id,name,starts_on,ends_on,is_current) values ('10000000-0000-0000-0000-000000000099','2025/2026','2025-09-01','2026-07-31',false);
...
-- Use the authenticated headmistress command boundary so these fixtures obey
-- immutable creator identity, draft-only creation, normalized targets, and the
-- approved draft-to-published lifecycle.
set local role authenticated;
select set_config('request.jwt.claim.sub','a0000000-0000-0000-0000-000000000002',true);
do $$ declare all_announcement uuid; class_announcement uuid; all_event uuid; class_event uuid; begin
  all_announcement := public.manage_internal_announcement(null,'draft','All targets','x','[{"target_kind":"school","role_code":null,"class_group_id":null},{"target_kind":"role","role_code":"parent","class_group_id":null},{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  perform public.manage_internal_announcement(all_announcement,'publish','All targets','x','[{"target_kind":"school","role_code":null,"class_group_id":null},{"target_kind":"role","role_code":"parent","class_group_id":null},{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  class_announcement := public.manage_internal_announcement(null,'draft','A only','x','[{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  perform public.manage_internal_announcement(class_announcement,'publish','A only','x','[{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  all_event := public.manage_private_event(null,'draft','All targets','',now(),now()+interval '1 hour','[{"target_kind":"school","role_code":null,"class_group_id":null},{"target_kind":"role","role_code":"parent","class_group_id":null},{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  perform public.manage_private_event(all_event,'publish','All targets','',now(),now()+interval '1 hour','[{"target_kind":"school","role_code":null,"class_group_id":null},{"target_kind":"role","role_code":"parent","class_group_id":null},{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  class_event := public.manage_private_event(null,'draft','A only','',now(),now()+interval '1 hour','[{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  perform public.manage_private_event(class_event,'publish','A only','',now(),now()+interval '1 hour','[{"target_kind":"class","role_code":null,"class_group_id":"40000000-0000-0000-0000-000000000001"}]');
  if not exists(select 1 from public.announcements where id=all_announcement and created_by=auth.uid() and status='published' and published_at is not null)
    or not exists(select 1 from public.events where id=all_event and created_by=auth.uid() and status='published' and published_at is not null) then
    raise exception 'communications fixtures did not preserve manager creator or lifecycle';
  end if;
end $$;
reset role;
...
do $$ begin
  ...
  begin insert into public.announcement_targets select id, 'school', null, null from public.announcements where title='All targets' and created_by='a0000000-0000-0000-0000-000000000002'; raise exception 'duplicate school target accepted'; exception when unique_violation then null; end;
  begin insert into public.event_targets select id, 'role', null, null from public.events where title='All targets' and created_by='a0000000-0000-0000-0000-000000000002'; raise exception 'invalid event target accepted'; exception when check_violation then null; end;
