...
export default async function ChildResultsPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  if (guardianError) return <main className="mx-auto max-w-5xl space-y-6"><PortalHeader role="parent" eyebrow="Parent" title="Child results"><p>Results could not be loaded.</p></PortalHeader><UnavailableDashboardState /></main>;
  ...
  if (linkError) return <main className="mx-auto max-w-5xl space-y-6"><PortalHeader role="parent" eyebrow="Parent" title="Child results"><p>Results could not be loaded.</p></PortalHeader><UnavailableDashboardState /></main>;
  if (!link) notFound();
  ...
  if (error) return <main className="mx-auto max-w-5xl space-y-6"><PortalHeader role="parent" eyebrow="Parent" title="Child results"><p>Results could not be loaded.</p></PortalHeader><UnavailableDashboardState /></main>;
  return <main className="mx-auto max-w-5xl space-y-6"><PortalHeader role="parent" eyebrow="Parent" title={`${child?.first_name} ${child?.last_name}'s results`}><p>Only results the school has released for this linked child are shown.</p></PortalHeader>{results?.length ? <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><ul className="divide-y">{results.map((result) => { const assessment = Array.isArray(result.assessments) ? result.assessments[0] : result.assessments; const teaching = assessment && (Array.isArray(assessment.teacher_assignments) ? assessment.teacher_assignments[0] : assessment.teacher_assignments); const subject = teaching && (Array.isArray(teaching.subjects) ? teaching.subjects[0] : teaching.subjects); return <li key={result.id} className="py-3"><strong>{assessment?.title}</strong><p>{result.score}/{assessment?.maximum_score}{subject ? ` · ${subject.code} — ${subject.name}` : ""}</p><p className="text-sm text-slate-700">{assessment?.assessment_date}</p>{result.feedback && <p className="mt-1 text-sm text-slate-700">{result.feedback}</p>}</li>; })}</ul></section> : <EmptyDashboardState title="No released results">There are no released results for this child.</EmptyDashboardState>}</main>;
}
