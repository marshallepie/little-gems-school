...
function UnavailableTeacherDashboard() {
  return <main className="mx-auto max-w-6xl space-y-6"><PortalHeader role="teacher" eyebrow="Teacher dashboard" title="Your teaching overview"><p>Your school information could not be loaded right now.</p></PortalHeader><UnavailableDashboardState /></main>;
}
...
export default async function TeacherPage() {
  ...
  if (!teacher || teacher.employment_status !== "active") return <main className="mx-auto max-w-6xl space-y-6"><PortalHeader role="teacher" eyebrow="Teacher dashboard" title="Your teaching overview"><p>Your active teacher record is not linked to this account.</p></PortalHeader><EmptyDashboardState title="No active teaching record linked">Ask an administrator to link and activate your school teacher record before assignments and class rosters can appear here.</EmptyDashboardState></main>;
  ...
  return <main className="mx-auto max-w-6xl space-y-6">
    <PortalHeader role="teacher" eyebrow="Teacher dashboard" title={`Welcome, ${fullName(teacher.first_name, teacher.last_name)}`}><p>Only your active class assignments and the students enrolled in those classes are shown.</p></PortalHeader>
    ...
    <nav className="flex flex-wrap gap-3" aria-label="Teacher workflow links"><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/teacher/assignments">Assignments</Link><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/teacher/assessments">Assessments and draft results</Link><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/teacher/attendance">Attendance</Link></nav><FutureFeatureNote>Lesson plans and messaging are not available in this phase.</FutureFeatureNote>
  </main>;
}
