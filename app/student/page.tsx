...
function UnavailableStudentDashboard() {
  return <main className="mx-auto max-w-4xl space-y-6"><PortalHeader role="student" eyebrow="Student dashboard" title="Your school overview"><p>Your school information could not be loaded right now.</p></PortalHeader><UnavailableDashboardState /></main>;
}
...
export default async function StudentPage() {
  ...
  if (!student) return <main className="mx-auto max-w-4xl space-y-6"><PortalHeader role="student" eyebrow="Student dashboard" title="Your school overview"><p>Your student record is not linked to this account yet.</p></PortalHeader><EmptyDashboardState title="No student record linked">Ask an administrator to link your school student record before your class information can appear here.</EmptyDashboardState></main>;
  ...
  return <main className="mx-auto max-w-4xl space-y-6">
    <PortalHeader role="student" eyebrow="Student dashboard" title={`Hello, ${fullName(student.first_name, student.last_name)}`}><p>This is your own school identity and active class information.</p></PortalHeader>
    ...
    <nav className="flex flex-wrap gap-3" aria-label="Student workflow links"><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/student/assignments">My assignments</Link><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/student/results">My results</Link><Link className="min-h-11 rounded border px-4 py-3 font-semibold" href="/student/attendance">My attendance</Link></nav><FutureFeatureNote>Lessons and messaging are not available in this phase.</FutureFeatureNote>
  </main>;
}
